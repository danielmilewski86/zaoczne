...
describe("About Higher Order Functions", function () {
  ...
  it("should use 'map' to transform each element", function () {
    ...
    expect(numbersPlus1).toEqual([2,3,4]); //dodaje jeden na koncu tablicy i tworzy nowa tablice
    expect(numbers).toEqual([1,2,3]); //niezmieniona tablica
  });
  ...
  it("should use 'reduce' to update the same result on each iteration", function () {
    ...
    expect(reduction).toBe(6); // dodalo alementy tablicy do siebie
    expect(numbers).toEqual([1,2,3]); // tablica jest nie zmieniona
  });
